...
func NewConn(ctx context.Context, target string) (*grpc.ClientConn, error) {
	...
	return grpc.DialContext(ctx, target, grpc.WithChainUnaryInterceptor(otelgrpc.UnaryClientInterceptor(), clientInterceptor), grpc.WithDefaultServiceConfig(`{"loadBalancingConfig": [{"round_robin":{}}]}`), grpc.WithInsecure())
}
